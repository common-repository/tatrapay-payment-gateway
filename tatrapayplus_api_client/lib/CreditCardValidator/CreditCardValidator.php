...
namespace Tatrapayplus\TatrapayplusApiClient\CreditCardValidator;
...
class CreditCardValidator
{
    ...
    const TYPE_DINERS_CLUB = 'DN';
    const TYPE_DISCOVER = 'DI';
    const TYPE_JCB = 'JCB';
    const TYPE_UNIONPAY = 'UN';
    const TYPE_MAESTRO = 'MI';
    const TYPE_ELO = 'ELO';
    const TYPE_MIR = 'mir';
    const TYPE_HIPER = 'hiper';
    const TYPE_HIPERCARD = 'HC';
	 ...
	 * @return string
	 */
	public function getTypeString($cardNumber)
	{
		$cardTypeObj = $this->getType( $cardNumber );
		if ( $cardTypeObj ) {
			return $cardTypeObj->getType();
		} else {
			return static::TYPE_MASTERCARD; #fallback
		}
	}
