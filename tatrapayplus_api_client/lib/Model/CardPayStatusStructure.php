...
class CardPayStatusStructure implements ModelInterface, \ArrayAccess
{
    ...
    public function setCurrency($currency)
    {
        ...
        if (!preg_match('/[A-Z]{3}/', ObjectSerializer::toString($currency))) {
            throw new SanitizedInvalidArgumentException('invalid value for $currency when calling CardPayStatusStructure., must conform to the pattern /[A-Z]{3}/.');
        }
    ...
    public function setMaskedCardNumber($masked_card_number)
    {
        ...
        if (mb_strlen($masked_card_number) > 19) {
            throw new SanitizedInvalidArgumentException('invalid length for $masked_card_number when calling CardPayStatusStructure., must be smaller than or equal to 19.');
        }
