...
class AccountReference implements ModelInterface, \ArrayAccess
{
    ...
    {
        if (is_null($iban)) {
            throw new SanitizedInvalidArgumentException('non-nullable iban cannot be null');
        }

        if (!preg_match('/[A-Z]{2,2}[0-9]{2,2}[a-zA-Z0-9]{1,30}/', ObjectSerializer::toString($iban))) {
            throw new SanitizedInvalidArgumentException('invalid value for $iban when calling AccountReference., must conform to the pattern /[A-Z]{2,2}[0-9]{2,2}[a-zA-Z0-9]{1,30}/.');
        }
